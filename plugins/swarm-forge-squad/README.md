...
>
> ```bash
> skills/swarm-forge-squad/scripts/install_squad.sh <project-dir>
> cd <project-dir> && ./swarm
> ```
>
> That copies the payload (roster, role prompts, worker templates, contracts, project
> templates, constitution) *and the engine* into the project, instead of extracting a branch
> tarball, and pre-seeds the Acceptance Pipeline tooling into squad's tool cache from the
> bundled copy. **The installed project holds no path back into this plugin**, so it keeps
...
> documented below.
>
> Squad forks the SwarmForge engine — 86 of its own scripts and its own `squadd` dashboard —
> so it is a separate plugin from `swarm-forge`, which carries the pack workflows.
>
> **Vendored from** `https://github.com/unclebob/swarm-forge`, branch `squad`, commit
> `b0d63d5` (2026-08-21). The upstream rewrite-planning documents (`redo.md`, `redo-ui.md`,
> `redo-implementation-plan.md`) were left out; everything else is as-is.
>
> Also vendored: `https://github.com/unclebob/Acceptance-Pipeline-Specification` at
> `accaa33` (2026-06-12), under `vendor/`.
>
> Running `bb test` here needs a JDK in addition to Babashka, because `bb.edn` resolves a
> git dependency through the Clojure CLI.
>
...
>
> - Upstream's own suite, unmodified: `bb test` runs 425 non-simulation tests, exit 0.
>   Note it requires a `.swarmforge/roles.tsv` to exist at the repo root or it aborts with
>   "Cannot find SwarmForge project root" before running anything — a pre-existing upstream
>   quirk, not a packaging artifact.
> - `squad_tool` `require` and `ensure` resolving the vendored APS tooling offline, including
>   rebuilding a deleted wrapper without touching the network.
> - Install smoke tests: payload, engine, contracts, tool cache, and no path back into the
>   plugin.
>
> Not exercised: a live squad swarm, and `bb simulation-test`, which fails upstream — its
> simulator shells out to `swarmforge/scripts/squad_theme.sh`, a file that does not exist on
> the `squad` branch. The `swarmforge/` and `test/` trees here are byte-identical to upstream
> `b0d63d5`, so that failure is upstream's, not this packaging's.
