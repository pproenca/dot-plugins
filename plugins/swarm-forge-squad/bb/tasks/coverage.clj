...
;; Requires deps.edn :cov alias (Cloverage).
;; This repo is not a product swarm; Cloverage still calls project-root,
;; which needs a gitignored .swarmforge/roles.tsv marker.
...
(def roles-marker ".swarmforge/roles.tsv")

(defn coverage-ok? []
  (and (fs/exists? lcov-path)
       (pos? (fs/size lcov-path))))

(defn ensure-project-root-marker! []
  (when-not (fs/exists? roles-marker)
    (fs/create-dirs (fs/parent roles-marker))
    (spit roles-marker
          (str "squad-leader\tmaster\t" (fs/cwd)
  ...
  (cond
    ...
    (do
      (binding [*out* *err*]
        ...
        (println "Configure :cov in deps.edn and ensure tests run under Cloverage.")
