...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a Codex task id or URL, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. Use a Codex task id or URL, an exact transcript path supplied by the environment, or a pushed branch. Read the task summary and latest turns first, then scan back for decisions. Parse a long task in a subagent and keep the reduced timeline in the main thread.
