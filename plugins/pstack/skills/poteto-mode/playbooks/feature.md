...
6. For consequential changes with coupled state transitions or ordering rules, preserve review separation: have an agent who did not implement the change compare the public contract with both code and test expectations. Reuse an existing lead/worker split or use one bounded reviewer under the Codex collaboration contract. Resolve substantiated discrepancies and rerun affected checks. Review the diff for unsupported guards, dead compatibility paths, unrelated changes, and comments that only narrate code. Use **interrogate** when a broader adversarial review is needed.
