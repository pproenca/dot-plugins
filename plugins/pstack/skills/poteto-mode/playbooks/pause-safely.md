...
1. Stop at a safe boundary. Finish the current atomic step or back out of it. Never stop mid-edit in a known-broken state. Start nothing new. Use `interrupt_agent` when it is available to stop each nested agent's current turn. If interruption is unavailable, record every running agent and do not claim a clean stop.
