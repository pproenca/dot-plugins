...
6. **Verify the chain from task history, not self-report.** Read each candidate's task through the Codex task tools, or use the exact transcript path when the environment provides one. Never scan unrelated workspaces. Check which files each candidate opened. Grade chain-following from those reads and the code, never from the candidate's claims.
