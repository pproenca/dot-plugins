...
1. **Verify every PR independently before arming anything.** One collaboration subagent per PR exercises the real product through an available browser, computer-use, or CLI control skill against parent versus head. Each returns `PASS`, `PASS+NOTES`, or `FAIL` and posts that verdict on its PR. Safe means a verdict from an agent that did not write the code. CI green and bot approval are not verdicts.
...
8. **Watch the drain, do not drive it.** Arm the watcher in queued mode over the verified run. Use a thread heartbeat automation for fallback wakeups and re-arm the watcher after any verdict you act on, until COMPLETE at the ceiling. ADVANCE is progress, not termination. Report each merge and the new ceiling. Diagnose a stall before mutating.
