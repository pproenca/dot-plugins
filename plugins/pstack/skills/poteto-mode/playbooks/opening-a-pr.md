...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as `control-cli`, a browser-control skill, computer-use, or the targeted tests. State the outcome of each check, not only the command name.
...
**Readiness.** Open every PR ready, never as a draft. Pass the ready-state option supported by the selected PR tool. If a PR still opens as a draft, run the host's ready command, such as `gh pr ready <number>`. Run `gh pr view <number>` before you refer to PR status.
