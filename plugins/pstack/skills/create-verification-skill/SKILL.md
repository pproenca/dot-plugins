...
description: "Generate a project-local verification skill that drives your app the way a user does — any language, framework, or platform. Use when $pstack:create-verification-skill, \"make a control skill for this repo\", or when a project has no scripted way to prove UI/CLI/service behavior."
---

# Create a verification skill

Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates a project-local Codex skill under `.agents/skills/verify-<app>/`. Write the output for the next agent, not for a human. It will be read cold by an agent that has never seen the app.
...
Write `.agents/skills/verify-<app>/SKILL.md` with YAML frontmatter. Use `name: verify-<app>` and a description that names the app, product, and trigger. Ground every section below in what the repository interview found. Leave no placeholders.
