...
The September 6 efficiency revision removes remaining fixed exploration panels and seven-source searches from ordinary How and Why questions. Independent parallel work remains encouraged when it can save time or improve confidence. Explicit broad investigations and consequential review still expand to the risks the task exposes. Why retains precise citations, competing hypotheses, and the distinction between absent evidence and an unsuccessful search; it no longer requires a separate synthesizer just to write an answer.

The collaboration contract now carries a task base revision and owned paths through review, including task commits and uncommitted work. It encourages bounded tool results, batched independent checks, and event waits while retaining evidence needed to finish the task. Autonomous runs resolve blocking discoveries but do not acquire unrelated side projects. These changes follow Astra's guidance on task completion, instruction conflicts, delegation, and proportionate verification; they impose neither a context-size target nor a lower parent reasoning level. Personal worker overrides remain separate from the portable defaults and must respect the user's effort choices.

The evaluation protocol now includes lead and worker usage, review and repair, context size per request, and cost per accepted outcome. Cached input and reasoning subsets must not be counted twice, and repeated cumulative counters are not individual requests. A high cache-hit rate alone is not an efficiency result. Source simplification and routing checks do not establish a percentage saving or quality equivalence across all tasks.
