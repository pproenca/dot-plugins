...
You don't need to name a playbook or list skills. "repro first" and a checkable outcome are all the routing signal `/poteto-mode` needs. It matches the Bug fix playbook, records the phases through the planning tool or normal progress updates, and calls the right skills as each step fires.
