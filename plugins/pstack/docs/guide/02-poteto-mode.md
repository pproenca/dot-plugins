...
`$pstack:poteto-mode` is the front door. You give it a goal, it matches one of twenty-two playbooks, keeps the playbook phases visible, and calls the other skills as the steps need them. Pstack uses the current planning capability when one is available and normal progress updates otherwise. In this page you learn what a good prompt looks like, and how little of one you actually need.
