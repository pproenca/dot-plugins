<!-- Added when vendoring into the dot-plugins marketplace. -->
...
> tarball over your project, after which `./swarm` downloads the engine from GitHub. A
> plugin cannot be several branches, so the four rosters are vendored here as data under
> `packs/` and the download is replaced by an offline install:
>
> ```bash
> skills/swarm-forge/scripts/install_pack.sh <two-pack|four-pack|six-pack|adversaries> <project-dir>
> cd <project-dir> && ./swarm
> ```
>
> The install produces the same project layout upstream's bootstrap produces — the pack
> payload, the engine at `swarmforge/scripts/`, `close-swarm`, and a `./swarm` — plus the
> Acceptance Pipeline tooling pre-seeded under `.swarmforge/`, which upstream clones from
> GitHub on first use. **No step needs network, and the installed project holds no path back
...
> `bb test` runs unmodified from this directory.
>
> The `squad` workflow is a separate plugin, `swarm-forge-squad`; it shares no code with
> these packs.
>
> **Vendored from** `https://github.com/unclebob/swarm-forge`:
>
> | Source | Commit | Dated |
> | --- | --- | --- |
> | `main` (engine, dashboard, shared articles) | `ba50410` | 2026-08-22 |
> | `two-pack` -> `packs/two-pack` | `fc53139` | 2026-08-21 |
> | `four-pack` -> `packs/four-pack` | `615a083` | 2026-08-22 |
> | `six-pack` -> `packs/six-pack` | `b933d68` | 2026-08-22 |
> | `adversaries` -> `packs/adversaries` | `7aa2f3a` | 2026-06-26 |
> | `unclebob/Acceptance-Pipeline-Specification` -> `vendor/` | `accaa33` | 2026-06-12 |
>
> To re-vendor, pull each branch again at the paths above — the roster branches are not
> reachable from this directory's history.
>
> Neither upstream repository states a license, so no `license` field is declared in
> `plugin.json`.
>
> ### What has been exercised
>
> - Upstream's own suite, unmodified, from this directory: 119 tests / 459 assertions, 0
>   failures — including 67 pack-cockpit tests and 24 assertions against `dashboard.html`.
> - A live `two-pack` swarm on `codex`, end to end: startup, git repo and worktree creation,
>   both tmux sessions, the cockpit serving and reporting both lanes, an agent resolving
>   `gherkin-parser` through `swarm_tool.sh require` against the pre-seeded tooling, a real
>   commit, a `git_handoff` queued with `swarm_handoff.sh`, `handoffd` delivering it to the
>   cleaner's inbox, the board card moving `coder -> cleaner`, and teardown via
>   `./close-swarm`.
> - The installed project's `commit-msg` hook was confirmed to point inside the project, not
>   at this plugin.
>
> Not exercised: the `adversaries` pack's visible-window terminal adapters, the `grok` and
> `copilot` backends, and the four-pack/six-pack specifier approval gate.
>
> ### Patched upstream defects
>
> Two bugs in `swarmforge/scripts/pack_web.bb`, both present in upstream `main`, are fixed
> here. See [`patches/`](patches/) for the diff and the full write-up.
>
> 1. **The cockpit could not type into agents.** `pane-target` hardcoded tmux pane `.0`,
>    while the launcher correctly detects `pane-base-index`. On any host whose tmux sets
>    `pane-base-index 1`, New Task, chat, and clarification answers silently did nothing —
>    `inject-role!` swallowed the failure and the API still returned `{"ok":true}`. The fix
>    asks the running tmux server for the setting, so it works on any machine; it defaults to
>    `0`, so upstream's own tests pass unmodified.
> 2. **Teardown could stop halfway, silently.** `run-teardown!` ran its four steps with no
>    isolation and `schedule-teardown!` discarded the exception before exiting, so a failure
>    in the first step left the agents and the handoff daemon running while the dashboard
...
> up a real tmux server with `pane-base-index 1` and asserts a cockpit task actually reaches
> the pane.
